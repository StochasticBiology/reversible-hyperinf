...
for(expt in 0:35) {
  ...
  plotHypercube.curated.tree(my.ct, scale.fn = NULL, hjust = 1) +
...
quick.labs = c("sp", "tp", "sp-", "tp-", "uni", "soft", "soft-", "loss")
df_pca2$group = c(1:length(model.fits), 100, 101, 102, 103:107, 108:112)
df_pca2$rgroup = c(quick.labs[1+(1:length(model.fits)-1) %% 8], "SP-1", "SP-2", "TP", paste("RND", 1:5), paste("ORD", 1:5))
...
summary(pca2)$importance["Proportion of Variance", ]
