...
for(expt in 0:35) {
  ...
  model.fits[[length(model.fits)+1]] = list(my.hct.out)
  ...
  sf = 2
  fname = paste0("tester-large-", L, "-", tree.size, "-", expt, ".png", collapse="")
  png(fname, width=1200*sf, height=600*sf, res=72*sf)
  print(ggarrange(plotHypercube.curated.tree(my.ct), ggarrange(
                                 phct, p.hct, ncol=1, nrow=2), widths=c(1,2.5)))
  dev.off()
}

save(model.fits, file=paste0("model-large-fits-", L, "-", tree.size, ".Rdata", collapse=""))

m.list = list()
m.names = c()
for(i in 1:length(model.fits)) {
  m.list[[i]] = pull_orders_hct(model.fits[[i]][[1]]$routes)
  m.names = c(m.names, paste(i-1, c("ht")))
  diag(m.list[[i]]) = NA
}
...
sf = 2
png("large-outputs.png", width=800*sf, height=800*sf, res=72*sf)
...
true.m[[2]] = matrix(0.5, nrow=L, ncol=L)
...
df_pca2$group = c(1:length(model.fits), 100, 101)
df_pca2$rgroup = c((1:length(model.fits)-1) %% 7, "one", "two")
...
png("large-outputs-truth.png", width=800*sf, height=800*sf, res=72*sf)
pca2.plot
dev.off()
